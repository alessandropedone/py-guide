...
class PolynomialCV:
    ...
    def refit_best(self) -> make_pipeline:
        ...
        self.best_model = make_pipeline(PolynomialFeatures(degree=self.best_deg), LinearRegression())
    ...
    def plot(self, true_func=None):
        ...
        plt.plot(self.X, y_pred, color="darkorange", linestyle="--", label=f"Degree {self.best_deg} fit")
        if true_func is not None:
            plt.plot(self.X, true_func(self.X), color="green", linestyle=":", label="True curve")
    ...
    def summary(self):
        ...
        for deg, r in self.results.items():
            ...
            print(f"{deg:<5} {r['val_r2']:>8.2f} {r['val_r2_std']:>10.2f} {r['train_r2']:>10.2f} {r['val_rmse']:>10.2f}{marker}")
